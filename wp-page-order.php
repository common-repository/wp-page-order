...
Plugin URI: http://www.ds-appdevelopment.com
Description: Simple Page Ordering - New Version Support Latest WP Version 4.4 . Order your pages and other hierarchical post types with simple drag and drop right from the standard page list.

Version: 1.0.0
Author: Jaklin
Author URI: http://www.ds-appdevelopment.com
License: GPLv2 or later
*/

if ( ! class_exists( 'WP_Page_Order' ) ) :
...
class WP_Page_Order {
	...
	public static function _add_actions() {
		...
		add_action( 'wp_ajax_wp_page_order', array( __CLASS__, 'ajax_wp_page_order' ) );
	...
	public static function load_textdomain() {
		load_plugin_textdomain( 'wp-page-order', false, dirname( plugin_basename( __FILE__ ) ) . '/localization/' ); 
	}
	...
	public static function wp() {
		...
		if ( ( is_string( $orderby ) && 0 === strpos( $orderby, 'menu_order' ) ) || ( isset( $orderby['menu_order'] ) && $orderby['menu_order'] == 'ASC' ) ) {
			$script_name = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? 'wp-page-order.dev.js' : 'wp-page-order.js';
			wp_enqueue_script( 'wp-page-order', plugins_url( $script_name, __FILE__ ), array('jquery-ui-sortable'), '2.1', true );
			wp_enqueue_style( 'wp-page-order', plugins_url( 'wp-page-order.css', __FILE__ ) );
		}
	}
	...
	public static function admin_head() {
		...
		$screen->add_help_tab(array(
			...
			'content' => '<p>' . __( 'To reposition an item, simply drag and drop the row by "clicking and holding" it anywhere (outside of the links and form controls) and moving it to its new position.', 'wp-page-order' ) . '</p>',
		));
	}

	public static function ajax_wp_page_order() {
	...
	public static function sort_by_order_link( $views ) {
		...
		$views['byorder'] = sprintf('<a href="%s" class="%s">%s</a>', $query_string, $class, __("Sort by Order", 'wp-page-order'));
...
if (false) {
	// Plugin description
	__('Order your pages and hierarchical post types using drag and drop on the built in page list. For further instructions, open the "Help" tab on the Pages screen.', 'wp-page-order');
}
